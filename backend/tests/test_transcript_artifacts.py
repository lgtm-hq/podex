...
from podex.services.episode_iterator import EpisodeIterator, ProcessingStage
...
def test_artifact_backed_transcript_remains_eligible_for_cleanup(
    db_session: Session,
) -> None:
    """Verify batch processing sees retained encrypted payload metadata."""
    episode = _create_episode(db_session)
    store = RecordingArtifactStore()
    persist_transcript_acquisition(
        db=db_session,
    ...
    pending = EpisodeIterator(db_session).get_pending(ProcessingStage.CLEANUP)

    assert_that(pending).contains_only(episode)
