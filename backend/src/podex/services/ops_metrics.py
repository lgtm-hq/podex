...
@dataclass(frozen=True, slots=True)
class ProjectionLagData:
    """Search projection backlog health."""
...
def get_operational_metrics(
    *,
    ...
    pending_repairs = (
        ...
        .filter(
            SearchProjectionRepair.status == SearchProjectionRepairStatus.PENDING.value
        )
        .all()
    )
    oldest_pending_age_seconds = None
    if pending_repairs:
        oldest_created_at = min(item.created_at for item in pending_repairs)
        if oldest_created_at.tzinfo is None:
            oldest_created_at = oldest_created_at.replace(tzinfo=UTC)
        oldest_pending_age_seconds = max(
            0,
            int((measured_at - oldest_created_at).total_seconds()),
        )
    delivered_digests = (
    ...
    return OperationalMetricsData(
        ...
        projection=ProjectionLagData(
            ...
            failed_repairs=db.query(SearchProjectionRepair)
            .filter(
